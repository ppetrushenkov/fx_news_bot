...
def set_targets(data: pd.DataFrame, look_forward_bars: int):
    ...
    - `trg_bar_volatility_expansion_gradation` - returns the maximum degree of volatility
        for any of the next N bars;
    - `trg_is_greater_daily_atr` - returns True if the range of the next N candles (in total)
        forms a range greater than the Daily ATR;  (TO BE CONFIRMED)
    (RANGE REGRESSION)
    - `trg_future_range_n(hour)` - range of the following bars:
        - 1 hour;
        - 3 hours;
        - 6 hours
    (CHAOS CLASSIFICATION)
    - `trg_big_doji` - returns True if there will be a large Doji (or pin bar) candle in the next 2 hours.
        In essence, a candle with the body to the entire candle ratio below 0.2 and the candle size greater than ATR;
    - `trg_dir_changes` - count of direction changes after news release
        (number of sequential updates of extrema).
        Also known as sequential stop loss breaks in both directions;
    - `trg_is_flat_flg` - minimum value of KER (Kaufman Efficiency Ratio) on the following N (8 to 24) bars.
        Should be less than 0.1;
    - `trg_is_trend_flg` - maximum value of KER on the following N (8 to 24) bars. Should be greater than 0.5;
    - `trg_is_chaos` - complex label, meaning a change in direction (trg_dir_changes) > 1,
        volatility > 3 times ATR and simultaneously should be both trend and flat.
    """
    # Prerequisites
    atr = data['atr']
    ...
    # target_columns = [i for i in data.columns if i.startswith('trg_')]
