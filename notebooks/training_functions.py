...
def run_time_series_cv_catboost(
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        # for fold, (train_idx, val_idx) in enumerate(
        #     RollingWindowCV(window=365*5*48, horizon=365*48, step=365*48).split(df), 1
        # ):
            ...
            if task_type == "regression":
                ...
                mlflow.log_metric("Precision", precision_score(y_val, y_pred), step=fold)
                mlflow.log_metric("Recall", recall_score(y_val, y_pred), step=fold)
